...
import aiohttp,re,os,requests
from src.config.appconfig import SERPER_API_KEY
from pydantic.v1 import BaseModel, root_validator
from langchain_core.utils import get_from_dict_or_env
from typing_extensions import Literal

class StockPriceFinder(BaseModel):
    k: int = 10
    gl: str = "us"
    hl: str = "en"
...
def stock_price_finder(name:str,description:str):
    stockPriceFinder = StockPriceFinder(serper_api_key=SERPER_API_KEY)
    return Tool(
        name=name,
        description=description,
        func=stockPriceFinder.run,
        coroutine = stockPriceFinder.arun,
    )